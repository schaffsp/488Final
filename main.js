...
async function initialize() {
  ...
  /*
    vec3 normal = normalize(mixNormal);
      vec4 texel = texture(grassTexture, mixTexPosition) * vec4(0.5, 0.5, 0.5, 1.0 - opacity);
      vec2 offTexPosition = vec2(mixTexPosition.s - 0.2, mixTexPosition.t - 0.2);
      texel += texture(grassTexture, offTexPosition) * vec4(1.0, 1.0, 1.0, opacity / 2.0);
      offTexPosition = vec2(mixTexPosition.s - 0.4, mixTexPosition.t - 0.4);
      texel += texture(grassTexture, offTexPosition) * vec4(1.0, 1.0, 1.0, opacity / 2.0);
      fragmentColor = texel * vec4(0.3, 0.3, 0.3, 1.0);
  */
